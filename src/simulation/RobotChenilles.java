...
public class RobotChenilles extends Robot {
	...
	/**
	 * Vitesse du robot en conditions normales.
	 */
	 ...
	 * 
	 */
	public RobotChenilles(long position, CarteSynchronized carte) {
	 ...
	 */
	public RobotChenilles(long position, CarteSynchronized carte, int eau) {
	...
	public int getVitesse(TypeArc arc) {
		int vit = 0;
		if (arc == TypeArc.NORMAL || arc == TypeArc.ESCARPE) {
			vit = VITESSENORMALE * 2;
		}
		if (arc == TypeArc.INONDE) {
			vit = VITESSENORMALE / 2;
		}
		return vit;
	}
